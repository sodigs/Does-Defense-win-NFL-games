...
import os
import numpy as np
def14=pd.read_csv('/Users/saumi/Documents/Python Projects/2014 Team DVOA Ratings Defense.csv')
def14
r2014
merge2014=pd.merge(def14,r2014,how="left",on="Teams")
print(merge2014)
...
DVOAdf= merge2014[['Teams','Total DVOA Rank', 'Pass DVOA Rank', 'Rush DVOA Rank', 'WinPct']]
DVOAdf
DVOAdf.corr('pearson')
...
perc=merge2014[['Teams','Total DVOA', 'Pass DVOA', 'Rush DVOA', 'WinPct']]
...
plt.ylabel('Win %')
plt.title("2014: Defense vs Win Pct")
m,b=np.polyfit(edperc['Total_DVOA'],edperc['WinPct'].map(floater),1)
lin14=plt.plot(edperc['Total_DVOA'],m*edperc['Total_DVOA']+b)
...
jut14=edperc.iloc[:,1:5].corr()
jut14
heat14=sns.heatmap(jut14,vmin=-1,vmax=1,cmap='viridis',annot=True,linewidth=0.1)

#%%
#Multiple Linear Regression Model | Total DVOA + +Rush DVOA + Pass DVOA Logilikelihood=16.797, .463, .405
